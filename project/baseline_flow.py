...
class BaselineNLPFlow(FlowSpec):
    ...
    def baseline(self):
        ...
        "Compute the baseline - predict most common class"

        ### TODO: Fit and score a baseline model on the data, log the acc and rocauc as artifacts.
        self.valdf['pred'] = 0
        self.base_acc = accuracy_score(self.valdf.label, self.valdf.pred)
        self.base_rocauc = accuracy_score(self.valdf.label, self.valdf.pred)
    ...
    def end(self):
        ...
        self.false_positives = self.valdf[(self.valdf['pred'] ==1) & (self.valdf['label'] == 0)]
        # TODO: display the false_positives dataframe using metaflow.cards
        current.card.append(Table.from_dataframe(self.false_positives))
        # Documentation: https://docs.metaflow.org/api/cards#table

        current.card.append(Markdown("## Examples of False Negatives"))
        # TODO: compute the false positive predictions where the baseline is 0 and the valdf label is 1.
        self.false_negatives = self.valdf[(self.valdf['pred'] ==0) & (self.valdf['label'] == 1)]
        # TODO: display the false_negatives dataframe using metaflow.cards
        current.card.append(Table.from_dataframe(self.false_negatives))
